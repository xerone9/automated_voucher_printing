...
def find_and_print(rfid_code):
    ...
    student_all_voucher_data = get_pending_vouchers(rfid_code)
    if student_all_voucher_data:
        if not checkbox_var.get():
            for voucher in student_all_voucher_data:
                if voucher[-1]:
                    rfid = voucher[0]
                    KUICKPAY_MESSAGE_ON = False
                    with open("settings.ini", "r") as file:
                        for line in file:
                            value = line.strip()
                            if value.__contains__("KUICKPAY_MESSAGE = "):
                                kuickpay_message_boolean = value.split("KUICKPAY_MESSAGE = ")[1]
                                if str(kuickpay_message_boolean) == "1":
                                    KUICKPAY_MESSAGE_ON = True
                    if KUICKPAY_MESSAGE_ON:
                        print("Hello")
                        try:
                            print(kuickpay_sms(voucher[1]))
                        except Exception as e:
                            pass
                    break
            else:
                rfid = student_all_voucher_data[0][0]
        else:
            rfid = student_all_voucher_data[0][0]
    else:
        rfid = None
    ...
    if rfid:
        ...
        if voucher_no not in ALREADY_PRINTED_VOUCHER or checkbox_var.get():
            ...
            try:
                ...
                command = ["SumatraPDF-3.5.2-64.exe", '-print-to-default', '-print-settings', '1',  file_location + file_name]
