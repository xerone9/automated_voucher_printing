...
def print_job_checker(retry_limit=2, retry_delay=1):
    ...
    while jobs:
        ...
        try:
            ...
            error_message = str(e)
            print(error_message)
            if "data area passed to a system call is too small" in error_message:
                retry_count += 1
                if retry_count > retry_limit:
                    print(f"Retry limit reached ({retry_limit}). Exiting.")
                    return None
                else:
                    print(f"Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)  # Wait before retrying
                    continue  # Retry the current loop
            else:
